...
def generate_calibration_report():
    ...
    # 2. 指定数据目录路径（相对于 src 目录）
    raw_dir = os.path.join(os.path.dirname(__file__), "..", "data", "raw")
    calib_dir = os.path.join(os.path.dirname(__file__), "..", "data", "calibrated")

    # 收集标定前后的文件用于画图
    raw_files = sorted(glob.glob(os.path.join(raw_dir, "RAW_S*.csv")))
    calib_files = sorted(glob.glob(os.path.join(calib_dir, "CALIB_S*.csv")))

    if not raw_files or not calib_files:
        print("❌ 未找到 RAW_S*.csv 或 CALIB_S*.csv 文件，请检查数据目录！")
    ...
    for file in raw_files:
        ...
        # 从文件名提取活动名称，如 RAW_S01_RUNNING_MOCK_01.csv -> RUNNING_MOCK_01
        filename = os.path.basename(file)
        activity_name = filename.replace("RAW_", "").replace(".csv", "")
    ...
    for file in calib_files:
        ...
        # 从文件名提取活动名称，如 CALIB_S01_RUNNING_MOCK_01.csv -> RUNNING_MOCK_01
        filename = os.path.basename(file)
        activity_name = filename.replace("CALIB_", "").replace(".csv", "")
    ...
    sample_calib = pd.read_csv(calib_files[0])
...
"""
    ...
    # 报告输出到 src 同级目录
    report_path = os.path.join(os.path.dirname(__file__), "标定报告.md")
